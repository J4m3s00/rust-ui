...
const OPEN_SANS_TTF: &[u8] = include_bytes!("../fonts/Roboto-Regular.ttf");
...
impl Application {
    fn new(
        ...
        paths.insert(
            0,
            contrast_renderer::path::Path::from_rounded_rect([0.0, 0.0], [5.8, 1.3], 0.5),
        );
        paths[0].stroke_options = Some(contrast_renderer::path::StrokeOptions {
            ...
            curve_approximation: contrast_renderer::path::CurveApproximation::UniformTangentAngle(
                0.1.into(),
            ),
        });
        let shape = contrast_renderer::renderer::Shape::from_paths(
            &device,
            &renderer,
        ...
        Self {
            ...
            dynamic_stroke_options,
            instance_buffers: [
                contrast_renderer::renderer::Buffer::new(
                    device,
    ...
    fn resize(
        ...
        if MSAA_SAMPLE_COUNT > 1 {
            ...
            self.msaa_color_texture_view = Some(msaa_color_texture.create_view(
                &wgpu::TextureViewDescriptor {
                    dimension: Some(wgpu::TextureViewDimension::D2),
                    ..wgpu::TextureViewDescriptor::default()
                },
        ...
        self.renderer.resize_internal_buffers(
            device,
            self.viewport_size,
    ...
    fn render(
        ...
        self.shape
            .set_dynamic_stroke_options(queue, 0, &self.dynamic_stroke_options[0])
            .unwrap();
        let projection_matrix = contrast_renderer::utils::matrix_multiplication(
            ...
            &contrast_renderer::utils::motor3d_to_mat4(
                &Translator::new(1.0, 0.0, 0.0, -0.5 * self.view_distance)
                    .geometric_product(self.view_rotation),
            ),
        );
        const ROWS: usize = 9;
        const COLUMNS: usize = 5;
        let mut instances_transform: Vec<[geometric_algebra::ppga3d::Point; 4]> =
            Vec::with_capacity(1 + ROWS * COLUMNS);
        let mut instances_color: Vec<contrast_renderer::renderer::Color> =
            Vec::with_capacity(1 + ROWS * COLUMNS);
        ...
        self.instance_buffers[0].update(
            device,
            queue,
            &contrast_renderer::concat_buffers!([&instances_transform]).1,
        );
        self.instance_buffers[1].update(
            device,
            queue,
            &contrast_renderer::concat_buffers!([&instances_color]).1,
        );
        ...
        match event {
            winit::event::WindowEvent::CursorMoved { position, .. } => {
                let position = [
                    std::f32::consts::PI
                        * 1.2
                        * (position.x as f32 / self.viewport_size.width as f32 - 0.5),
                    std::f32::consts::PI
                        * 1.2
                        * (position.y as f32 / self.viewport_size.height as f32 - 0.5),
                ];
                self.view_rotation =
                    contrast_renderer::utils::rotate_around_axis(position[0], &[0.0, 1.0, 0.0])
                        .geometric_product(contrast_renderer::utils::rotate_around_axis(
                            position[1],
                            &[1.0, 0.0, 0.0],
                        ));
            }
            winit::event::WindowEvent::MouseWheel { delta, .. } => {
                let difference = match delta {
                    ...
                    winit::event::MouseScrollDelta::PixelDelta(delta) => {
                        [delta.x as f32 * 0.1, delta.y as f32 * 0.1]
                    }
                };
